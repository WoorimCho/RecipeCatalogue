...
| | |
|---|---|
| `GET /` | `?name=`, `?tag=` (+`?match=all\|any`), `?notTag=` (repeatable — exclude; ANDed with `tag`), `?ingredientId=` (repeatable), paged, `?sort=` (`name` / `creator` / `version`). Tag terms match a tag name **anywhere** — `?tag=vegan` finds `diet:vegan` recipes |
