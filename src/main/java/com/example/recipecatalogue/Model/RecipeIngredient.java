...
/**
 * A catalogue entry used by a {@link Recipe}, with its role in that recipe.
 *
 * <p>{@code ingredientId} is an id in the Ingredient catalogue (no FK).
...
public class RecipeIngredient {
    ...
    /**
     * Free-text amount for display ("2 cups, packed", "a good pinch"); null if
     * unspecified. The machine-readable pair is {@link #amount} + {@link #unit}.
     */
    @Setter
    @Column(name = "quantity", length = 100)
    private String quantity;

    /** Numeric quantity, paired with {@link #unit}. Null when only free-text {@link #quantity} is given. */
    @Setter
    @Column(name = "amount")
    private Double amount;

    /** Unit for {@link #amount}. Null iff {@code amount} is null. */
    @Setter
